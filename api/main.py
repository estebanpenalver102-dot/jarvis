...
async def get_admin_token(request: "Request"):
    """
    Returns admin token in partial form.
    Full token only revealed over HTTPS from the Render Logs on first boot.
    """
    from fastapi import Request
    from security import _ADMIN_TOKEN
    token = _ADMIN_TOKEN
    if not token:
        return {"error": "Admin token not yet initialized"}
    # Only show partial token here (full token is in Render logs)
    return {
        "hint": f"{token[:8]}...{token[-8:]}",
        "note": "Full token was logged at startup — check Render service logs (Logs tab)."
    }
